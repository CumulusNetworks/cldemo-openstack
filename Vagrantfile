...
#    using topology data from: cldemo-no-oob.dot
#    built with the following args: ./topology_converter.py -p libvirt -a cldemo-no-oob.dot
#
...
# ANSIBLE GROUPS CONFIGURATION
    ansible.groups = {
      ...
      "host" => ["server01","server03","server02","server04",],
      "leaf" => ["leaf04","leaf02","leaf03","leaf01",],
  ...
  config.vm.define "spine02" do |device|
    ...
    device.vm.provision "file", source: "./config/#{device.vm.hostname}/interfaces", destination: "~/interfaces"
    device.vm.provision "file", source: "./config/#{device.vm.hostname}/daemons", destination: "~/daemons"
    device.vm.provision "file", source: "./config/#{device.vm.hostname}/frr.conf", destination: "~/frr.conf"
...
udev_rule
     device.vm.provision :shell , :inline => <<-udev_rule
echo "  INFO: Adding UDEV Rule: 44:38:39:00:00:1d --> swp31"
echo 'ACTION=="add", SUBSYSTEM=="net", ATTR{address}=="44:38:39:00:00:1d", NAME="swp31", SUBSYSTEMS=="pci"' >> /etc/udev/rules.d/70-persistent-net.rules
udev_rule
     device.vm.provision :shell , :inline => <<-udev_rule
echo "  INFO: Adding UDEV Rule: 44:38:39:00:00:17 --> swp32"
echo 'ACTION=="add", SUBSYSTEM=="net", ATTR{address}=="44:38:39:00:00:17", NAME="swp32", SUBSYSTEMS=="pci"' >> /etc/udev/rules.d/70-persistent-net.rules
udev_rule
...
udev_rule
     device.vm.provision :shell , :inline => <<-udev_rule
echo "  INFO: Adding UDEV Rule: 44:38:39:00:00:07 --> swp49"
echo 'ACTION=="add", SUBSYSTEM=="net", ATTR{address}=="44:38:39:00:00:07", NAME="swp49", SUBSYSTEMS=="pci"' >> /etc/udev/rules.d/70-persistent-net.rules
udev_rule
     device.vm.provision :shell , :inline => <<-udev_rule
